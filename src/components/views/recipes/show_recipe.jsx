...
const RecipeTitle = (props) => {
  ...
  return (
    <div>
      <h1>{props.name}</h1>
      <h5>Submitted by <button className='author-text' onClick={() => props.sortByUser(props.author.id)}>{props.author.username}</button>. {props.url && <a href={props.url} className='author-text' target='_blank'>View Original Recipe</a> }</h5>
      {RecipeTags}
    </div>
  )
}
    ...
    return (
      <div className='show-recipe-container show-recipe-style'>
        ...
        <div className='show-recipe-buttons form-group'>
            <button className='btn btn-success btn-md' onClick={(event) => this.addToGroceryList(event)}><i className="fas fa-shopping-bag"></i> Add To Grocery List</button>
          </div>
        </div>
      </div>
    )
